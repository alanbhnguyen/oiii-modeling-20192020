...
def flag_spec(fit_parameters, wave, fluxden, error):
    ...
    elif fit_parameters[0][3] == 0: #second gaussian gives zero thus single
        FLAG = 1

    elif fit_parameters[0][2] > 1900: #second gaussian un realistically wide thus single
        FLAG = 1

    elif fit_parameters[0][5] > 1900: #second gaussian un realistically wide thus single
        FLAG = 1

    elif fit_parameters[0][2] < 0: #negative width?? essentially flat
        FLAG = 1

    elif fit_parameters[0][5] < 0: #negative width?? essentially flat
        FLAG = 1

    elif fit_parameters[0][1] < min(to_vel_OIII(wave)): #fits second gauss elsewhere for somereason??
        FLAG = 1

    elif fit_parameters[0][4] < min(to_vel_OIII(wave)): #fits second gauss elsewhere for somereason??
        FLAG = 1

    elif fit_parameters[0][1] > max(to_vel_OIII(wave)): #fits second gauss elsewhere for somereason??
        FLAG = 1

    elif fit_parameters[0][4] > max(to_vel_OIII(wave)): #fits second gauss elsewhere for somereason??
        FLAG = 1
        
    elif fit_parameters[0][0] < 0: #negative amp bad data
        FLAG = 0

    elif fit_parameters[0][3] < 0: #negative amp bad data
        FLAG = 0

    elif fit_parameters[0][2] < -1000: #second gaussian un realistically wide thus single
        FLAG = 1

    elif fit_parameters[0][5] < -1000: #second gaussian un realistically wide thus single
        FLAG = 0

    elif rcsq > 20: #poor fit bad data
        FLAG = 0
